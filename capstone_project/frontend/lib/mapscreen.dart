  ...
  Future<void> _fetchBuildings() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> buildingJson = json.decode(
          utf8.decode(response.bodyBytes),
        );
        setState(() {
          _buildings = buildingJson
  ...
  Future<void> _fetchDirections() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // 경로 안내 텍스트 업데이트
        setState(() {
          _directionsSteps = (data['steps'] as List).map((stepData) {
            // stepData를 Map<String, dynamic>으로 명시적 캐스팅하거나,
            // 안전하게 dynamic으로 사용하면서 내부 접근 시 타입을 확인합니다.
            final step = stepData as Map<String, dynamic>; // 타입을 명확히 지정

            return {
              "instructions": _stripHtmlIfNeeded(
                step['html_instructions'] as String,
              ),
              // *** 여기를 수정해야 합니다. distance 객체의 'text' 필드를 사용합니다. ***
              // step['distance']는 Map이므로, 그 안의 'text' 키를 가져와야 합니다.
              "distance":
                  (step['distance'] as Map<String, dynamic>)['text'] as String,
            };
          }).toList();
        });
      } else {
